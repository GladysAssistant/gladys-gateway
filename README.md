...
"Is my Gladys alive?": the gateway is the only party able to tell a household that its home is unreachable (power cut, dead SD card, internet box down while on holidays), precisely because the home itself cannot. When the primary instance of an account has not been connected to the gateway for longer than the delay of the account (60 minutes by default), the confirmed admins of the account receive the "instance offline" email, once per outage; when the instance connects again, the "back online" email. The alert is **enabled by default** for every account: an admin of the account turns it off or changes the delay (between 5 minutes and 7 days) with `PATCH /accounts/instance-offline-alert` and `{ "enabled": false, "delay_in_minutes": 120 }`, from the Gladys Plus settings of Gladys; the current values are returned by `GET /users/me`.

The server runs the watchdog by itself every 5 minutes (`INSTANCE_WATCHDOG_CRON`, `*/5 * * * *` by default, `disabled` to turn it off), with the same Redis lock as the other jobs (held 4 minutes: a tighter schedule is pointless). `POST /admin/api/instances/watchdog` is the same job as a route, with the same read-only-by-default contract as the jobs above (`{ "execute": true }` to send the emails): call it without `execute` to review what the next run would do, or run it by hand. It compares the primary instances of the active accounts with the instances connected in websocket on the cluster, and refreshes `last_seen_at` of the connected ones (the websocket disconnect records it too, so the outage start is exact). The frequency only decides how late after the delay the email leaves: an instance is never reported offline while it is connected, whatever the interval, and an instance never seen by the gateway is never reported (so nothing is sent at the first deployment). The job fails closed: when none of the instances is connected while at least `INSTANCE_WATCHDOG_FAIL_CLOSED_MIN_INSTANCES` (10 by default) of them are checked, the socket cluster is suspect rather than every customer, and the run is aborted without any email (`"aborted": "no_instance_connected"` in the report).
