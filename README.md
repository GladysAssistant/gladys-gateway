...
- `POST /admin/api/accounts/activation-reminders` reminds the customers who subscribed through Stripe Checkout but never activated their Gladys Plus account. `ACCOUNT_ACTIVATION_REMINDER_DELAY_IN_DAYS` (7 by default) after the account was created (the welcome email is sent at that moment), the billing email receives one reminder with a fresh activation link, in the language of the checkout. An account is a candidate only while its subscription is running, it has no user and no reminder was sent yet: each account is reminded once. Meant to be called daily by a cron. On the first execution, every old account never activated is a candidate at once: review the list it returns without `execute` first.
...
Internal accounts (team, tests, demos) are flagged with `PATCH /admin/api/accounts/:id` and `{ "is_internal": true }`: they are excluded from the paying users stats and are never touched by the activation reminders nor by the retention policy.
