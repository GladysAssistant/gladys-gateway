...
| `MONDIAL_RELAY_ENSEIGNE` / `MONDIAL_RELAY_PRIVATE_KEY`                                                                                                                                                                                                     | API1 credentials, used to track the parcels (see below). Without them the parcels are simply not tracked automatically                              |
| `MONDIAL_RELAY_API2_LOGIN` / `MONDIAL_RELAY_API2_PASSWORD`                                                                                                                                                                                                 | API2 credentials, used to create the shipments and the labels. Without them, labels are created by hand and the tracking number is typed in the API |
| `MONDIAL_RELAY_API2_CUSTOMER_ID`                                                                                                                                                                                                                           | API2 customer id. Optional: defaults to the code enseigne                                                                                           |
| `MONDIAL_RELAY_API2_SANDBOX`                                                                                                                                                                                                                               | `true` to create the shipments on the API2 sandbox instead of production. The parcels it returns are not real                                       |
| `MONDIAL_RELAY_LABEL_FORMAT`                                                                                                                                                                                                                               | Label size: `10x15` (default), `A4` or `A5`                                                                                                         |
...
| `MONDIAL_RELAY_COLLECT_POINT_ID`                                                                                                                                                                                                                           | Pickup point where you drop the parcels, when the collect mode is `REL`. Optional: Mondial Relay picks it automatically                             |
...
Mondial Relay exposes **two different APIs**, with two different sets of credentials, and this integration uses both (technical documentation V1.2, February 2026):

| API      | Technology | Used for                    | URL                                                 |
| -------- | ---------- | --------------------------- | --------------------------------------------------- |
| **API1** | SOAP       | Parcel tracking             | `https://api.mondialrelay.com/WebService.asmx`      |
| **API2** | REST/XML   | Shipment and label creation | `https://connect-api.mondialrelay.com/api/shipment` |

The shipment creation methods of API1 (`WSI2_CreationEtiquette`) are no longer maintained by Mondial Relay and must not be used, which is why the two APIs are needed.

**API1 credentials** are in Connect under **Mon profil → Mes paramètres de connexion**: the _code enseigne_ (`MONDIAL_RELAY_ENSEIGNE`, 8 characters, also the `Brand` of the widget) and the _clé privée_ (`MONDIAL_RELAY_PRIVATE_KEY`). The _code marque_ and _code marque numérique_ shown next to them are not needed. Requests are signed with an MD5 hash of the parameters and of the private key. The API URL can be overridden with `MONDIAL_RELAY_API_URL`.

**API2 credentials** are generated in Connect under **Administration → Configuration des API → API Version V2.0**. The option only appears once the user has the "Configuration des API" right (Administration → Gestion des Utilisateurs), and after a logout/login. They are a login (`MONDIAL_RELAY_API2_LOGIN`, of the form `CODEENSEIGNE@business-api.mondialrelay.com`), a password (`MONDIAL_RELAY_API2_PASSWORD`) and a customer id (`MONDIAL_RELAY_API2_CUSTOMER_ID`, the code enseigne by default). There is no MD5 hash on API2.

Two limits of the **test account** (`TTMRSDBX`) are worth knowing: tracing is not available on it, and its private key can be changed by Mondial Relay without notice. Checking the tracking therefore needs the production account. API2 has its own sandbox (`MONDIAL_RELAY_API2_SANDBOX=true`), which creates labels flagged as such and no real parcel.

Once the variables are in `.env`, check both APIs with:

```sh
npm run check-mondial-relay
# with the number of a real shipment of the account, for a meaningful tracking answer:
npm run check-mondial-relay -- 12345678
```

It tracks a parcel on API1 and sends a request with no shipment in it to API2, so nothing is ever created. It tells whether each API accepts the credentials, or which variable is wrong (API1 `STAT=97`: private key, `STAT=1/2/3/69`: enseigne, `STAT=95`: tracing not enabled on this account; API2 `10001`: login or password, `10005`: customer id, `10066/10067`: account not set up for API2).

Tracing must stay occasional: Mondial Relay asks for no more than 4 to 6 calls a day per parcel, and none once it is delivered. The daily cron calls it once a day per shipped order and stops as soon as the parcel is delivered.
