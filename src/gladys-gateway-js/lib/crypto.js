...
module.exports = function({ cryptoLib }) {
  async function generateKeyPair() {
    ...
    var rsaPrivateKeyJwk = await cryptoLib.subtle.exportKey('jwk', rsaKeys.privateKey);
    ...
    var ecdsaPrivateKeyJwk = await cryptoLib.subtle.exportKey('jwk', ecdsaKeys.privateKey);
  ...
  async function encryptPrivateKeyJwk(passphrase, privateKeyJwk) {
    // sanitize passphrase
    passphrase = sanitizePassPhrase(passphrase);
  ...
  async function decryptPrivateKeyJwk(passphrase, wrappedKey, type, salt, iv) {
    iv = Uint8Array.from(iv);
    salt = Uint8Array.from(salt);
