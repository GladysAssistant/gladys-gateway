module.exports = function UserController(userModel, mailService, socketModel, instanceModel) {
   ...
   * @apiParam {String} name Between 2 and 30 characters
   * @apiParam {String} email Email of the user
   * @apiParam {String} [two_factor_code] Current 2FA code, required to change the email when 2FA is enabled
   * @apiParam {string="en","fr"} language language of the user
   *
   * @apiSuccessExample {json} Success-Response:
   * HTTP/1.1 200 OK
  ...
  async function updateUser(req, res, next) {
    const {
      ...
      ...user
    } = await userModel.updateUser(req.user, req.body);

    if (emailConfirmationToken !== undefined) {
      ...
      // warn the previous address, so the legitimate owner is notified if it's not him
      mailService.send({ email: previousEmail, language: user.language }, 'email_changed', {
        newEmail: user.email,
      });
    }

    // the confirmation token is only sent to the new email address, never in the API response
    res.json(user);
  }
   ...
   * @apiGroup User
   *
   * @apiParam {String} two_factor_secret The new secret (from GET /users/two-factor/new)
   * @apiParam {String} two_factor_code A code generated by the 2FA app with the NEW secret
   * @apiParam {String} current_two_factor_code A code generated by the 2FA app with the CURRENT secret,
   * required when two factor is already enabled
   *
   * @apiSuccessExample {json} Success-Response:
   * HTTP/1.1 200 OK
   *
  ...
  async function updateTwoFactor(req, res, next) {
    res.json(
      await userModel.updateTwoFactor(
        req.user,
        req.body.two_factor_secret,
        req.body.two_factor_code,
        req.body.current_two_factor_code,
      ),
    );
  }
  ...
  /**
   * @api {post} /users/two-factor/recovery-codes Generate two factor recovery codes
   * @apiName Generate two factor recovery codes
   * @apiGroup User
   *
   * @apiDescription Generate a new set of single-use recovery codes. The codes
   * are only returned once, only a hash is stored. Any previously generated
  ...
  async function generateTwoFactorRecoveryCodes(req, res, next) {
    res.json(await userModel.generateTwoFactorRecoveryCodes(req.user, req.body.two_factor_code));
  }

  /**
   * @api {post} /users/login-recovery-code Login with two factor recovery code
   * @apiName Login with two factor recovery code
   * @apiGroup User
   *
   * @apiParam {String} two_factor_recovery_code A single-use recovery code generated by the user
   * @apiParam {String} device_name The name of the device that will be displayed in the UI of logged device
   *
   * @apiSuccessExample {json} Success-Response:
   * HTTP/1.1 200 OK
   ...
   * }
   */
  async function loginTwoFactorRecoveryCode(req, res, next) {
    const tokens = await userModel.loginTwoFactorRecoveryCode(
      req.user,
   ...
   * @apiParam {String} [two_factor_recovery_code] A single-use 2FA recovery code, as an alternative to the 2FA code
   *
   * @apiSuccessExample {json} Success-Response:
   * HTTP/1.1 200 OK
   *
